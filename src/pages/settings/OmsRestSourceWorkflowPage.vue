...
async function save(): Promise<void> {
  ...
  try {
    ...
    const response = await settingsFacade.saveOmsRestSourceConfig({
      ...
      // still gates directly on it. Derive it from the OMS endpoint's enabled state so the two
      // never drift apart. The create wizard never renders the panel (no config exists yet to
      // scope endpoint rows to), so it keeps the pre-existing default of true.
      canReadOrders: !isEditing.value || form.enabledEndpoints.includes('OMS'),
    })
    ...
    // holds no real data yet, and writing it would wrongly disable every non-Orders endpoint
    // (recon/returns/transfer) on a brand-new config that should start fully enabled (an absent
    // access row already means enabled).
